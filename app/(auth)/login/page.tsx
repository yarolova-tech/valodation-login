...
const Login = ({ user }: PropsUser) => {
  const [userCurente, setUserCurente] = useState<User | null>(user ?? null);
  ...
  useEffect(() => {
    const { data: subscription } = supabase.auth.onAuthStateChange(
      (_event, session) => {
        setUserCurente(session?.user ?? null);
      },
    );
      ...
      <div className="w-full md:w-1/2 shadow-md p-4 flex justify-center items-center rounded-lg min-w-sm">
        <div className="flex justify-between items-center flex-col w-full max-w-lg min-w-sm">
          <div className="w-full flex flex-col items-start p-2">
            ...
            <div>Veuillez saisir vos identifiants pour vous connecter.</div>
          </div>
          ...
          <div className="w-full p-6">
            ...
            >
              <FcGoogle size={24} strokeWidth={1.5} /> Se connecter avec Google
            </button>
